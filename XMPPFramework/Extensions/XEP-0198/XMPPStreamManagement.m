...
- (id)initWithStorage:(id <XMPPStreamManagementStorage>)inStorage dispatchQueue:(dispatch_queue_t)queue
{
	if ((self = [super initWithDispatchQueue:queue]))
	{
		if ([inStorage configureWithParent:self queue:moduleQueue]) {
			...
			NSLog(@"%@: %@ - Unable to configure storage!", THIS_FILE, THIS_METHOD);
		}
...
- (void)enableStreamManagementWithResumption:(BOOL)supportsResumption maxTimeout:(NSTimeInterval)maxTimeout
{
	dispatch_block_t block = ^{ @autoreleasepool{
		...
		if (isStarted)
		{
			NSLog(@"Stream management is already enabled/resumed.");
			return;
		}
		if (enableQueued || enableSent)
		{
			NSLog(@"Stream management is already started (pending response from server).");
			return;
		}
...
{
	if (resumptionId == nil) {
		NSLog(@"%@: Cannot resume stream: resumptionId is nil", THIS_FILE);
		return NO;
	}
	if (lastDisconnect == nil) {
		NSLog(@"%@: Cannot resume stream: lastDisconnect is nil", THIS_FILE);
		return NO;
	}
	...
	{
		NSLog(@"%@: Cannot resume stream: invalid lastDisconnect - appears to be in future", THIS_FILE);
		return NO;
	}
	if ((uint32_t)elapsed > timeout) // too much time has elapsed
	{
		NSLog(@"%@: Cannot resume stream: elapsed(%u) > timeout(%u)", THIS_FILE, (uint32_t)elapsed, timeout);
		return NO;
	}
...
- (void)sendResumeRequestWithResumptionId:(NSString *)resumptionId
{
	...
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		NSLog(@"%@: Attempting to resume: lastHandledByClient(%u) lastHandledByServer(%u)",
...
- (void)processResumed:(NSXMLElement *)resumed
{
	...
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		{
			NSLog(@"Unexpected h value from resume: lastH=%lu, newH=%lu, numPendingStanzas=%lu",
		...
		NSLog(@"%@: processResumed: lastHandledByServer(%u)", THIS_FILE, lastHandledByServer);
...
- (XMPPBindResult)handleBind:(NSXMLElement *)element withError:(NSError **)errPtr
{
	...
	else
	{
		if (![elementName isEqualToString:@"failed"]) {
			NSLog(@"%@: Received unrecognized response from server: %@", THIS_METHOD, element);
		}
...
- (void)processSentElement:(XMPPElement *)element
{
	...
	NSLog(@"%@: processSentElement (%@): lastHandledByServer(%u) pending(%lu)",
...
- (BOOL)processReceivedAck:(NSXMLElement *)ack
{
	...
	{
		NSLog(@"Error parsing h value from ack: %@", [ack compactXMLString]);
		return YES;
	}
	...
	{
		NSLog(@"Unexpected h value from ack: lastH=%lu, newH=%lu, numPendingStanzas=%lu",
		            (unsigned long)lastHandledByServer,
		            (unsigned long)h,
	...
	if (canProcessEntireAck || processed > 0)
	{
		if (canProcessEntireAck)
		{
			...
			NSLog(@"%@: processReceivedAck (fully processed): lastHandledByServer(%u) pending(%lu)",
		...
		else // if (processed > 0)
		{
			...
			NSLog(@"%@: processReceivedAck (partially processed): lastHandledByServer(%u) pending(%lu)",
			               THIS_FILE, lastHandledByServer, (unsigned long)[unackedByServer count]);
		}
	...
	{
		NSLog(@"%@: processReceivedAck (unprocessed): lastHandledByServer(%u) pending(%lu)",
		               THIS_FILE, lastHandledByServer, (unsigned long)[unackedByServer count]);
	}
...
- (void)xmppStreamDidFilterStanza:(XMPPStream *)sender
{
	...
	if (isStarted)
	{
		...
		NSLog(@"%@: xmppStreamDidFilterStanza: lastHandledByClient(%u) pendingToAck(%lu), pendingHandled(%lu)",
...
- (void)xmppStream:(XMPPStream *)sender didReceiveCustomElement:(NSXMLElement *)element
{
	...
	else if ([elementName isEqualToString:@"enabled"])
	{
		...
		{
			NSLog(@"Received unrequested <enabled/> stanza");
		}
	}
