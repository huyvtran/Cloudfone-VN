...
- (void)dealloc
{
	...
	{
		NSLog(@"%@: Deallocating prior to completion or cancellation. "
...
- (BOOL)xmppStream:(XMPPStream *)sender didReceiveIQ:(XMPPIQ *)iq
{
	...
	NSLog(@"%@: %@ - state(%i)", THIS_FILE, THIS_METHOD, state);
...
- (void)queryNextProxyCandidate
{
	...
	else
	{
		...
		else
		{
			...
			NSLog(@"%@: No proxies found", THIS_FILE);
...
- (void)socksConnect
{
	...
	NSLog(@"TURNSocket: hashMe : %@", hashMe);
	NSLog(@"TURNSocket: hashRaw: %@", hashRaw);
	NSLog(@"TURNSocket: hash   : %@", hash);
