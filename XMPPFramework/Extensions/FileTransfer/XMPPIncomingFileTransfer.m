...
- (instancetype)initWithDispatchQueue:(dispatch_queue_t)queue
{
    ...
    if (self) {
        ...
        listReceivedFile = [[NSMutableDictionary alloc] init];
    }
    return self;
}
...
- (void)dealloc
{
  ...
  if (_transferState != XMPPIFTStateNone) {
    NSLog(@"%@: Deallocating prior to completion or cancellation.", THIS_FILE);
  }
...
- (void)sendSIOfferAcceptance:(XMPPIQ *)offer
{
  ...
  dispatch_block_t block = ^{
      @autoreleasepool {
          NSXMLElement *siElement = [offer elementForName:@"si"];
          NSString *sidFile = [[siElement attributeForName:@"id"] stringValue];
          IncomingFileObj *aIncomingFile = [[IncomingFileObj alloc] init];
          
          // Store the sender's JID
          aIncomingFile._senderJID = offer.from;
        ...
        aIncomingFile._totalDataSize = [inFile attributeUnsignedIntegerValueForName:@"size"];

        // Store the name of the file for later use
        aIncomingFile._receivedFileName = [inFile attributeStringValueForName:@"name"];
...
//        } else {
          ...
          aIncomingFile._transferState = XMPPIFTStateWaitingForIBBOpen;
//        }
          aIncomingFile._idMessage = self.idMessage;
        ...
        [iq addChild:si];
          ...
          [listReceivedFile setObject:aIncomingFile forKey:sidFile];
        [xmppStream sendElement:iq];
      }
  };
  ...
  dispatch_block_t block = ^{
      @autoreleasepool {
        ...
        // Prepare to receive data
          NSXMLElement *openElement = [request elementForName:@"open"];
          if (openElement != nil) {
              NSString *sidFile = [[openElement attributeForName:@"sid"] stringValue];
              
              IncomingFileObj *curObject = [listReceivedFile objectForKey:sidFile];
              curObject._receivedData = [NSMutableData new];
          }
      }
  };
...
- (void)processReceivedIBBDataIQ:(XMPPIQ *)received
{
  ...
  dispatch_block_t block = ^{
      @autoreleasepool {
        ...
        NSData *temp = [[NSData alloc] initWithBase64EncodedString:dataElem.stringValue options:0];
          NSString *sidFile = [[dataElem attributeForName:@"sid"] stringValue];
          IncomingFileObj *curObject = [listReceivedFile objectForKey: sidFile];
          [curObject._receivedData appendData: temp];
          

        // According the base64 encoding, it takes up 4/3 n bytes of space, so
        // we need to find the size of the data before base64.
          curObject._receivedDataSize += (3 * dataElem.stringValue.length)/4;

          NSLog(@"Downloaded %lu/%lu bytes in IBB transfer.", (unsigned long) curObject._receivedDataSize, (unsigned long) curObject._totalDataSize);
        if (curObject._receivedDataSize < curObject._totalDataSize) {
          // Send ack response
          XMPPIQ *iq = [XMPPIQ iqWithType:@"result"
                                       to:received.from
          ...
          // We're finished!
          NSLog(@"Finished downloading IBB data.");
          [self transferSuccess:[[dataElem attributeForName:@"sid"] stringValue]];
        }
      }
  };
...
- (NSData *)sha1Hash
{
  ...
  NSLog(@"%@: hashMe : %@", THIS_FILE, hashMe);
  NSLog(@"%@: hashRaw: %@", THIS_FILE, hashRaw);
  NSLog(@"%@: hash   : %@", THIS_FILE, hash);
...
{
  //XMPPLogTrace();
  NSLog(@"Incoming file transfer failed because: %@", causeOfFailure);
...
*/
- (void)transferSuccess: (NSString *)sidFileStr
{
  //XMPPLogTrace();
    NSLog(@"transferSuccess");

  dispatch_block_t block = ^{
      @autoreleasepool {
        [self cancelIBBTimer];
          IncomingFileObj *curObject = [listReceivedFile objectForKey: sidFileStr];
          
        [multicastDelegate xmppIncomingFileTransfer:self
                                 didSucceedWithData:curObject._receivedData
                                              named:curObject._receivedFileName
                                          idMessage:curObject._idMessage];
          [listReceivedFile removeObjectForKey: sidFileStr];
        [self cleanUp];
...
- (BOOL)xmppStream:(XMPPStream *)sender didReceiveIQ:(XMPPIQ *)iq
{
    ...
    //jsi_658960257176284533
    
  if ([self isSIOfferIQ:iq]) {
  ...
  if ([self isIBBOpenRequestIQ:iq]) {
      NSXMLElement *openElement = [iq elementForName:@"open"];
      if (openElement != nil) {
          NSString *sidFile = [[openElement attributeForName:@"sid"] stringValue];
          IncomingFileObj *curObject = [listReceivedFile objectForKey: sidFile];
          if (curObject._transferState == XMPPIFTStateWaitingForIBBOpen) {
              [self sendIBBAcceptance:iq];
              curObject._transferState = XMPPIFTStateWaitingForIBBData;
              // Handle the scenario that the transfer is cancelled.
              [self resetIBBTimer:20];
              return YES;
          }
    ...
    if ([type isEqualToString:@"set"]) {
        NSXMLElement *element = [iq elementForName:@"close"];
        
        if (element != nil) {
            NSString *meStr = [[iq attributeForName:@"to"] stringValue];
            NSString *userStr = [[iq attributeForName:@"from"] stringValue];
            NSString *idResponse = [[iq attributeForName:@"id"] stringValue];
            
            XMPPIQ *iq = [XMPPIQ iqWithType:@"result"];
            [iq addAttributeWithName:@"from" stringValue: meStr];
            [iq addAttributeWithName:@"to" stringValue: userStr];
            [iq addAttributeWithName:@"id" stringValue:idResponse];
            [xmppStream sendElement:iq];
        }
    }
    if ([self isIBBDataIQ: iq]) {
        NSXMLElement *dataElement = [iq elementForName:@"data"];
        if (dataElement != nil) {
            NSString *sidFile = [[dataElement attributeForName:@"sid"] stringValue];
            IncomingFileObj *curObject = [listReceivedFile objectForKey: sidFile];
            if (curObject._transferState == XMPPIFTStateWaitingForIBBData) {
...
{
  NSLog(@"%@: didConnectToHost:%@ port:%d", THIS_FILE, host, port);
...
{
  NSLog(@"%@: didReadData:%@ withTag:%ld", THIS_FILE, data, tag);
...
{
  NSLog(@"%@: didWriteDataWithTag:%ld", THIS_FILE, tag);
      ...
      @autoreleasepool {
        ...
        for (NSXMLElement *streamhost in streamhosts) {
          ...
          if (![_asyncSocket connectToHost:host onPort:port error:&err]) {
            NSLog(@"%@: Unable to host:%@ port:%d error:%@", THIS_FILE, host, port, err);
            continue;
          }
...
- (void)socks5ReadMethod:(NSData *)incomingData
{
  ...
  dispatch_block_t block = ^{
      @autoreleasepool {
        ...
        NSLog(@"%@: writing connect request: %@", THIS_FILE, data);
      }
  };
