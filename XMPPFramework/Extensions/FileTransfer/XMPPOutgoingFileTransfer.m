...
*/
#define OUTGOING_DEFAULT_TIMEOUT 180
...
- (BOOL)sendData:(NSData *)data
     ...
     description:(NSString *)description
           error:(NSError **)errPtr  andIdMessage: (NSString *)idMessage
{
    if (_transferState != XMPPOFTStateNone) {
        if (errPtr) {
            NSString *errMsg = @"Transfer already in progress.";
    ...
    dispatch_block_t block = ^{
        @autoreleasepool {
            ...
            [file addAttributeWithName:@"size"
                           stringValue:[[NSString alloc] initWithFormat:@"%lu", (unsigned long) [_outgoingData length]]];//TODO
            [si addChild:file];
...
//            NSString *user = [MyFunctions getCallnexIDFromString: _recipientJID.user];
//            int expireTime = [NSDBCallnex getExpireTimeForUser: user];
            int expireTime = 0;
            if (expireTime > 0) {
                NSXMLElement *expire = [NSXMLElement elementWithName:@"expire" stringValue:[NSString stringWithFormat:@"%d", expireTime]];
                [file addChild:expire];
            }
      ...
      @autoreleasepool {
        ...
        if (errorElem) {
          ...
          if ([errType isEqualToString:@"modify"]
            ...
            NSLog(@"Responder prefers smaller IBB chunks. Shrinking block-size and retrying");
            _blockSize /= 2;
        ...
        if (iq.childCount == 0) {
          NSLog(@"Responder has accepted IBB session. Begin sending data");
          [self sendIBBData];
        }
      }
  };
  ...
  dispatch_block_t block = ^{
      @autoreleasepool {
        if (_sentDataSize < _totalDataSize) {
          ...
          NSLog(@"Uploading %lu/%lu bytes in IBB transfer.", (unsigned long) _sentDataSize,
          ...
          NSLog(@"IBB file transfer complete. Closing stream...");
...
- (void)handleIBBTransferQueryIQ:(XMPPIQ *)iq withInfo:(XMPPBasicTrackingInfo *)info
{
  ...
  dispatch_block_t block = ^{
      @autoreleasepool {
        ...
        NSLog(@"Received response signifying successful IBB stanza. Sending the next block");
      }
  };
  ...
  dispatch_block_t block = ^void {
      @autoreleasepool {
        ...
        if ([[value stringValue] isEqualToString:XMPPBytestreamsNamespace]) {
          NSLog(@"The recipient has confirmed the use of SOCKS5. Starting transfer...");
          [self collectStreamHosts];
        } else if ([[value stringValue] isEqualToString:XMPPIBBNamespace]) {
          NSLog(@"The recipient has confirmed the use of IBB. Beginning IBB transfer");
          [self beginIBBTransfer];
        }
      }
  };
  ...
  dispatch_block_t block = ^{
      @autoreleasepool {
        ...
        for (NSXMLElement *item in items) {
          ...
          if (itemJid) {
            NSLog(@"Found service %@. Querying to see if it's a proxy.", itemJid.full);
...
- (void)handleSentStreamhostsQueryIQ:(XMPPIQ *)iq withInfo:(XMPPBasicTrackingInfo *)info
{
    ...
    dispatch_block_t block = ^{
        ...
        NSLog(@"%@: streamhost-used received with jid: %@", THIS_FILE, jid);

        if ([jid isEqualToString:xmppStream.myJID.full]) {
            NSLog(@"%@: writing data via direct connection.", THIS_FILE);
        ...
        NSLog(@"%@: unable use a direct connection; trying the provided streamhost.", THIS_FILE);
...
- (void)handleSentActivateQueryIQ:(XMPPIQ *)iq withInfo:(XMPPBasicTrackingInfo *)info
{
    ...
    dispatch_block_t block = ^{
        @autoreleasepool {
            ...
            NSLog(@"Receive response to activate. Starting the actual data transfer now...");
...
- (NSData *)sha1Hash
{
    NSString *hashMe = [NSString stringWithFormat:@"%@%@%@", self.sid, xmppStream.myJID.full, _recipientJID.full];
...
{
    //XMPPLogTrace();
    NSLog(@"Outgoing file transfer failed because: %@", causeOfFailure);
...
- (void)socket:(GCDAsyncSocket *)sock didAcceptNewSocket:(GCDAsyncSocket *)newSocket
{
  ...
  NSLog(@"connected host: %@", newSocket.connectedHost);
  NSLog(@"connected port: %hu", newSocket.connectedPort);
...
{
  NSLog(@"%@: socket shouldTimeoutReadWithTag:%ld elapsed:%f bytesDone:%lu", THIS_FILE, tag, elapsed, (unsigned long)length);
...
shouldTimeoutWriteWithTag:(long)tag
                ...
                bytesDone:(NSUInteger)length
{
  NSLog(@"%@: socket shouldTimeoutWriteWithTag:%ld elapsed:%f bytesDone:%lu", THIS_FILE,
  ...
  dispatch_block_t block = ^{
      @autoreleasepool {
        ...
        if (version == 5 && method == 0) {
          ...
          NSLog(@"%@: writing SOCKS5 auth response: %@", THIS_FILE, responseData);
  ...
  dispatch_block_t block = ^{
      @autoreleasepool {
        ...
        if (![hash isEqualToData:[incomingData subdataWithRange:addrRange]]) {
          NSLog(@"Addresses don't match. Canceling the SOCKS5 transfer.");
        ...
        if (addrPort0 || addrPort1) {
          NSLog(@"Port should always be 0x00. Canceling the SOCKS5 transfer.");
