...
- (void)didCreateManagedObjectContext
{
	...
	if (messages == nil)
	{
		NSLog(@"%@: %@ - Error executing fetchRequest: %@", [self class], THIS_METHOD, error);
		return;
	}
                                                    ...
                                                    managedObjectContext:(NSManagedObjectContext *)moc
{
	...
	{
		NSLog(@"%@: %@ - Error executing fetchRequest: %@", THIS_FILE, THIS_METHOD, fetchRequest);
	}
	else
	{
                                                  ...
                                                  managedObjectContext:(NSManagedObjectContext *)moc
{
	...
	if (results == nil)
	{
		NSLog(@"%@: %@ - Fetch request error: %@", THIS_FILE, THIS_METHOD, error);
		return nil;
	}
	else
	{
...
- (void)archiveMessage:(XMPPMessage *)message outgoing:(BOOL)isOutgoing xmppStream:(XMPPStream *)xmppStream
{
	...
	[self scheduleBlock:^{
		...
		{
			NSLog(@"Previous archivedMessage: %@", archivedMessage);
