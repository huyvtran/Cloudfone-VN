...
- (void)setKeepAliveWhitespaceCharacter:(char)keepAliveChar
{
	dispatch_block_t block = ^{
		...
		{
			NSLog(@"Invalid whitespace character! Must be: space, newline, or tab");
		}
	};
...
- (void)startConnectTimeout:(NSTimeInterval)timeout
{
	...
	if (timeout >= 0.0 && !connectTimer)
	{
		...
		dispatch_source_set_cancel_handler(connectTimer, ^{            
            NSLog(@"%@: dispatch_release(connectTimer)", THIS_FILE);
...
- (BOOL)connectP2PWithSocket:(GCDAsyncSocket *)acceptedSocket error:(NSError **)errPtr
{
	...
	dispatch_block_t block = ^{ @autoreleasepool {
        ...
        [multicastDelegate xmppStream:self socketWillConnect:asyncSocket];
...
- (void)sendOpeningNegotiation
{
	...
	if (parser == nil)
	{
		NSLog(@"%@: Initializing parser...", THIS_FILE);
	...
	{
		NSLog(@"%@: Resetting parser...", THIS_FILE);
...
- (void)continueStartTLS:(NSMutableDictionary *)settings
{
	...
	NSLog(@"%@: %@ %@", THIS_FILE, THIS_METHOD, settings);
...
- (void)handleAuth:(NSXMLElement *)authResponse
{
	...
	{
		NSLog(@"Authentication class (%@) returned invalid response code (%i)",
...
- (void)socket:(GCDAsyncSocket *)sock didConnectToHost:(NSString *)host port:(UInt16)port
{
    ...
    self.enableBackgroundingOnSocket = YES;
	...
	#if TARGET_OS_IPHONE
	{
		if (self.enableBackgroundingOnSocket)
		{
			...
			if (result)
				NSLog(@"%@: Enabled backgrounding on socket", THIS_FILE);
			else
				NSLog(@"%@: Error enabling backgrounding on socket!", THIS_FILE);
		}
	}
	#endif
	
	[multicastDelegate xmppStream:self socketDidConnect:sock];
    [multicastDelegate xmppStream:self socketWillConnect:sock];
...
//    [self setEnableBackgroundingOnSocket: YES];
//}
                                    ...
                                    completionHandler:(void (^)(BOOL shouldTrustPeer))completionHandler
{
	...
	{
		NSLog(@"%@: Stream secured with (GCDAsyncSocketManuallyEvaluateTrust == YES),"
...
- (void)socket:(GCDAsyncSocket *)sock didWriteDataWithTag:(long)tag
{
	...
	if (tag == TAG_XMPP_WRITE_RECEIPT)
	{
		if ([receipts count] == 0)
		{
			NSLog(@"%@: Found TAG_XMPP_WRITE_RECEIPT with no pending receipts!", THIS_FILE);
			return;
		}
	...
	if (state == STATE_XMPP_CONNECTED)
	{
		if (keepAliveInterval > 0)
		{
			...
			dispatch_source_set_cancel_handler(keepAliveTimer, ^{
				NSLog(@"dispatch_release(keepAliveTimer)");
