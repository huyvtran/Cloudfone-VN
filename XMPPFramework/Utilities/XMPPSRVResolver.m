...
- (void)succeed
{
	...
	dispatch_async(delegateQueue, ^{ @autoreleasepool {
		...
		{
			NSLog(@"%@: delegate doesn't implement %@", THIS_FILE, NSStringFromSelector(selector));
		}
...
{
	NSLog(@"%@: %@ %i", THIS_FILE, THIS_METHOD, (int)sdErr);
...
- (XMPPSRVRecord *)processRecord:(const void *)rdata length:(uint16_t)rdlen
{
	...
	if (rdata == NULL)
	{
		NSLog(@"%@: %@ - rdata == NULL", THIS_FILE, THIS_METHOD);
		return nil;
	}
...
- (void)startWithSRVName:(NSString *)aSRVName timeout:(NSTimeInterval)aTimeout
{
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		NSLog(@"%@: startWithSRVName:%@ timeout:%f", THIS_FILE, aSRVName, aTimeout);
		...
		dispatch_source_set_event_handler(sdReadSource, ^{ @autoreleasepool {
			...
			NSLog(@"%@: sdReadSource_eventHandler", THIS_FILE);
		...
		dispatch_source_set_cancel_handler(sdReadSource, ^{ @autoreleasepool {
			...
			NSLog(@"%@: sdReadSource_cancelHandler", THIS_FILE);
